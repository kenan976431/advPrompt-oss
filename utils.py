...
def llm_loader(llm_params, verbose=False):
    ...
    if llm_params.dtype == "float32":
        ...
        dtype = torch.float16
    elif llm_params.dtype == "bfloat16":
    ...
    if llm_params.checkpoint == "stas/tiny-random-llama-2":
        ...
        # Set environment variable for memory allocation and default dtype
        import os
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            torch_dtype=dtype,
            device_map=getattr(llm_params, "device_map", llm_params.device),
            load_in_8bit=getattr(llm_params, "load_in_8bit", False),
            offload_folder=getattr(llm_params, "offload_folder", None),
            max_memory=getattr(llm_params, "max_memory", None),
        )
